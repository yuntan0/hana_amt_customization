frappe.pages['weekly-lead-report'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Weekly Lead Report',
		single_column: true
	});
	frappe.weekly_lead_report.make(page);
	frappe.weekly_lead_report.run();
}
frappe.weekly_lead_report = {
	start: 0,
	make: function(page) {
		var me = frappe.weekly_lead_report;
		me.page = page;
	...
	run: function(comapny,status) {
		var me = frappe.weekly_lead_report;
		frappe.call({
			method: 'hana_amt_customization.hana_amt_customization.page.weekly_lead_report.weekly_lead_report.get_data',
			args: {
				start: me.start,
				company : comapny,
				status:status
	...
	add_row: function(data) {
		...
		$(frappe.render_template('weekly_lead_report_row', data)).appendTo(me.body);
