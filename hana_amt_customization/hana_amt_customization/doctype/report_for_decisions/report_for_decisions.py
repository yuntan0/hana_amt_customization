...
class ReportForDecisions(Document):
		...
		if self.workflow_state == 'Confirmed':
			...
			for approval in self.approval_line:
				if approval.user_id == frappe.session.user and approval.approved == 0:
					...
					if not frappe.db.exists('ToDo',{'reference_type':'Report For Decisions', 'reference_name':self.name , 'owner': approval.user_id }):
						todo = frappe.new_doc('ToDo')
						todo.owner = approval.user_id
						todo.reference_type = 'Report For Decisions'
			...
			print("################################ Rejected ##################################")
			frappe.db.set_value('RFD Approval Lines', approval.name, 'approved', 1)
			frappe.db.set_value('RFD Approval Lines', approval.name, 'approve_date', frappe.utils.nowdate())
			frappe.db.set_value('RFD Approval Lines', approval.name, 'approve_status', 'Rejected')
			self.reload()
			...
			for approval in self.approval_line:
				if len(self.get("approval_line")) == approval.idx and approval.user_id == frappe.session.user:
					...
					frappe.db.set_value('RFD Approval Lines', approval.name, 'approved', 1 )
					frappe.db.set_value('RFD Approval Lines', approval.name, 'approve_date', frappe.utils.nowdate() )
					frappe.db.set_value('RFD Approval Lines', approval.name, 'approve_status', 'Approved')
					self.reload()
