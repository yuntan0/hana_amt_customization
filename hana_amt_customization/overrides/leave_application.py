...
class HAMTLeaveApplication(Document):
	def get_feed(self):
	...
	def validate_balance_leaves(self):
		if self.from_date and self.to_date:
			if frappe.db.get_value('Leave Type',self.leave_type,"is_hour")==0:
...
def get_leaves_for_period(
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if frappe.db.get_value('Leave Type',leave_type,"is_hour")==0:
				leave_days += (
				...
				leave_days += (
					...
					* -1 * int(frappe.db.get_value("Leave Application", {"name": leave_entry.transaction_name}, ["hours"]))
				)
